...
feature "View" do
  ...
  context "private wiki" do
    scenario "when not logged in is not successfull" do
      ...
      expect(page).to have_content('You must be logged in to view this private wiki')
    end
    
    scenario "after successfull login you get redirected to the wiki you tried to view" do
      visit wiki_path(private_wiki)
      expect(page).to have_content('You must be logged in to view this private wiki')
